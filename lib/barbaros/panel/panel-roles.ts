...
// Keys are matched case-insensitively against config.sector.
const SECTOR_MANAGER_TITLES: Array<{
  match: RegExp
  en: string
  ar: string
}> = [
  { match: /educat|school|teach|تعليم|مدرس/i, en: 'School Principal',        ar: 'مدير المدرسة' },
...
const SECTOR_EXPERT_TITLES: Array<{
  ...
  { match: /educat|school|teach|تعليم|مدرس/i, en: 'Head of Department',          ar: 'رئيس القسم' },
...
export function isPanelPlan(plan: string): boolean {
  ...
  return p.includes('professional') || p.includes('executive') || p === 'pro' || p === 'expert'
}
...
export function resolvePanelForConfig(config: PanelConfigLike): ResolvedPanel {
  ...
  const managerTitle =
    SECTOR_MANAGER_TITLES.find(t => t.match.test(sector))?.[lang] ??
    PANEL_ROLES.direct_manager.title[lang]

  const expertTitle = (
    SECTOR_EXPERT_TITLES.find(t => t.match.test(sector))?.[lang] ??
